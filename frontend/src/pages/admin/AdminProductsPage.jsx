    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="font-serif text-3xl sm:text-4xl text-[var(--color-botanical-primary)] tracking-tight font-normal">Products</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
          <div className="bg-[var(--color-surface-lowest)] p-4 rounded-xl border border-[var(--color-botanical-border)] shadow-xs">
            ...
            <div className="text-3xl font-serif font-medium text-[var(--color-botanical-primary)] leading-none">{products.length}</div>
          </div>
          <div className="bg-[var(--color-surface-lowest)] p-4 rounded-xl border border-[var(--color-botanical-border)] shadow-xs">
            <div className="flex items-center justify-between text-[var(--color-botanical-subtle)] mb-1.5">
          ...
          <div className="bg-[var(--color-surface-lowest)] p-4 rounded-xl border border-[var(--color-botanical-border)] shadow-xs">
            ...
            <div className="text-3xl font-serif font-medium text-[var(--color-botanical-primary)] leading-none">{categories.length - 1}</div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map(product => (
              <Link key={product.id} to={`/admin/products/${product.id}`}
                className="bg-[var(--color-surface-lowest)] rounded-xl border border-[var(--color-botanical-border)] shadow-xs hover:shadow-md hover:border-[var(--color-botanical-border)] transition-all overflow-hidden group">
                <div className="aspect-[4/3] bg-[var(--color-surface-low)] overflow-hidden">
                ...
                <div className="p-4 space-y-2">
                  ...
                  <h3 className="font-serif text-[15px] text-[var(--color-botanical-primary)] font-medium leading-snug line-clamp-2">{product.name}</h3>
                  <div className="flex items-center justify-between pt-2 border-t border-[var(--color-botanical-border-light)]">
                    <div>
                      <span className="text-[16px] font-bold text-[var(--color-botanical-primary)]">{formatINR(product.price)}</span>
                      {product.originalPrice && <span className="text-[11px] text-[var(--color-botanical-subtle)] line-through ml-1.5">{formatINR(product.originalPrice)}</span>}
                    </div>
                    <span className="text-[10px] text-[var(--color-botanical-subtle)] font-mono">{product.sku}</span>
                  </div>
                  <div className="flex items-center gap-2 pt-1">
                    <span className={`w-1.5 h-1.5 rounded-full ${product.visibility === 'Hidden' ? 'bg-[#ba1a1a]' : 'bg-[#5b6d54]'}`}></span>
                    <span className="text-[11px] text-[var(--color-botanical-subtle)]">{product.visibility === 'Hidden' ? 'Hidden' : 'Visible'}</span>
                  </div>
                </div>
              </Link>
            ))}
        ...
        {/* Footer */}
        <div className="text-center text-[12px] text-[var(--color-botanical-subtle)] pt-4">
          All prices in INR (₹)
        </div>
      </div>
    </AdminLayout>
