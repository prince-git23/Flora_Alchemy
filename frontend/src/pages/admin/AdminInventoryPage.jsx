    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-5 pb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="bg-[var(--color-surface-lowest)] p-3.5 sm:p-4 rounded-xl border border-[var(--color-botanical-border)] shadow-xs">
            <div className="flex items-center justify-between text-[var(--color-botanical-subtle)] mb-1"><span className="text-[10px] sm:text-[11px] uppercase tracking-wider font-semibold">In Stock</span><span className="material-symbols-outlined text-[15px] text-[#5b6d54]">check_circle</span></div>
            <div className="text-2xl sm:text-3xl font-serif font-medium text-[#5b6d54] leading-none">{inStock}</div>
          </div>
          <div className="bg-[var(--color-surface-lowest)] p-3.5 sm:p-4 rounded-xl border border-[var(--color-botanical-border)] shadow-xs">
            <div className="flex items-center justify-between text-[var(--color-botanical-subtle)] mb-1"><span className="text-[10px] sm:text-[11px] uppercase tracking-wider font-semibold">Low Stock</span><span className="material-symbols-outlined text-[15px] text-[#964735]">warning</span></div>
            <div className="text-2xl sm:text-3xl font-serif font-medium text-[#964735] leading-none">{lowStock}</div>
          </div>
          <div className="bg-[var(--color-surface-lowest)] p-3.5 sm:p-4 rounded-xl border border-[var(--color-botanical-border)] shadow-xs">
            <div className="flex items-center justify-between text-[var(--color-botanical-subtle)] mb-1"><span className="text-[10px] sm:text-[11px] uppercase tracking-wider font-semibold">Out / Critical</span><span className="material-symbols-outlined text-[15px] text-[#ba1a1a]">error</span></div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            <div className="relative flex-1 min-w-0">
              ...
              <input type="search" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search product or SKU..."
                className="w-full text-[13px] bg-[var(--color-surface-low)] border border-[var(--color-botanical-border)] focus:border-[#180f0a] rounded-lg pl-8 pr-3 py-1.5 text-[var(--color-botanical-text)] placeholder:text-[var(--color-botanical-subtle)] focus:ring-1 focus:ring-[#180f0a] transition" />
            </div>
            <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 sm:pb-0">
              {['all', 'In Stock', 'Low Stock', 'Critical', 'Out of Stock'].map(s => (
                <button key={s} type="button" onClick={() => setStatusFilter(s)}
                  className={`px-2.5 py-1 rounded-full text-[11px] font-medium whitespace-nowrap transition-all ${statusFilter === s ? 'bg-[#180f0a] text-white shadow-xs' : 'bg-[var(--color-surface-low)] text-[var(--color-botanical-muted)] hover:bg-[#ebe8e3]'}`}>
                  {s === 'all' ? 'All' : s}
                </button>
              ))}
            </div>
                      ...
                      <React.Fragment key={item.productId}>
                        <tr className={`hover:bg-[var(--color-surface-low)]/50 transition-colors ${expandedRow === item.productId ? 'bg-[var(--color-surface-low)]/30' : ''}`}>
                          ...
                          <td className="py-2.5 px-4 text-center">
                            <span className={`font-bold text-[14px] ${item.currentStock <= 0 ? 'text-[#ba1a1a]' : item.currentStock <= item.reorderLevel / 2 ? 'text-[#ba1a1a]' : item.currentStock <= item.reorderLevel ? 'text-[#964735]' : 'text-[var(--color-botanical-primary)]'}`}>
                              {item.currentStock}
                            </span>
                          </td>
                          ...
                          <td className="py-2.5 px-4 text-right">
                            <button type="button" onClick={() => { setExpandedRow(expandedRow === item.productId ? null : item.productId); setAdjustQty(''); setAdjustType('restock'); setAdjustReason(''); }}
                              className={`px-3 py-1 text-[11px] font-semibold rounded-full transition ${expandedRow === item.productId ? 'bg-[#180f0a] text-white' : 'text-[var(--color-botanical-primary)] bg-[var(--color-surface-low)] hover:bg-[#ebe8e3] border border-[var(--color-botanical-border)]'}`}>
                        ...
                        {expandedRow === item.productId && (
                          <tr>
                            <td colSpan={6} className="px-4 py-3 bg-[#faf8f5] border-b border-[var(--color-botanical-border)]">
                              <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 max-w-2xl">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex gap-1.5">
                                    <button type="button" onClick={() => setAdjustType('restock')}
                                      className={`flex-1 px-2.5 py-1.5 rounded-lg text-[12px] font-semibold border transition ${adjustType === 'restock' ? 'bg-emerald-50 text-emerald-700 border-emerald-300' : 'bg-[var(--color-surface-lowest)] text-[var(--color-botanical-muted)] border-[var(--color-botanical-border)] hover:bg-[var(--color-surface-low)]'}`}>
                                      + Add
                                    </button>
                                    <button type="button" onClick={() => setAdjustType('remove')}
                                      className={`flex-1 px-2.5 py-1.5 rounded-lg text-[12px] font-semibold border transition ${adjustType === 'remove' ? 'bg-[#ffdad3] text-[#783020] border-[#edd1cc]' : 'bg-[var(--color-surface-lowest)] text-[var(--color-botanical-muted)] border-[var(--color-botanical-border)] hover:bg-[var(--color-surface-low)]'}`}>
                                      − Remove
                                    </button>
                                  </div>
                                </div>
                                <div className="w-full sm:w-24">
                                  ...
                                  <input type="number" min="1" value={adjustQty} onChange={e => setAdjustQty(e.target.value)} placeholder="0"
                                    className="w-full text-[13px] bg-[var(--color-surface-lowest)] border border-[var(--color-botanical-border)] focus:border-[#180f0a] rounded-lg px-2.5 py-1.5 text-[var(--color-botanical-text)] focus:ring-1 focus:ring-[#180f0a] transition" />
                                </div>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <input type="text" value={adjustReason} onChange={e => setAdjustReason(e.target.value)} placeholder="Optional note"
                                    className="w-full text-[12px] bg-[var(--color-surface-lowest)] border border-[var(--color-botanical-border)] focus:border-[#180f0a] rounded-lg px-2.5 py-1.5 text-[var(--color-botanical-text)] placeholder:text-[var(--color-botanical-subtle)] focus:ring-1 focus:ring-[#180f0a] transition" />
                                </div>
                  ...
                  <div className="p-3.5">
                    ...
                    <div className="flex items-center justify-between mt-2.5">
                      <div className="flex items-center gap-2">
                        <span className={`text-[20px] font-serif font-medium ${item.currentStock <= 0 ? 'text-[#ba1a1a]' : item.currentStock <= item.reorderLevel / 2 ? 'text-[#ba1a1a]' : item.currentStock <= item.reorderLevel ? 'text-[#964735]' : 'text-[var(--color-botanical-primary)]'}`}>
                          {item.currentStock}
                        </span>
                        <span className="text-[11px] text-[var(--color-botanical-subtle)]">/ {item.reorderLevel} min</span>
                      </div>
                      <button type="button" onClick={() => { setExpandedRow(expandedRow === item.productId ? null : item.productId); setAdjustQty(''); setAdjustType('restock'); setAdjustReason(''); }}
                        className={`px-3 py-1.5 text-[11px] font-semibold rounded-full transition ${expandedRow === item.productId ? 'bg-[#180f0a] text-white' : 'text-[var(--color-botanical-primary)] bg-[var(--color-surface-low)] border border-[var(--color-botanical-border)]'}`}>
                        {expandedRow === item.productId ? 'Close' : 'Adjust'}
                      </button>
                    </div>
                  </div>
                  {expandedRow === item.productId && (
                    <div className="px-3.5 pb-3.5 pt-0 border-t border-[var(--color-botanical-border-light)] space-y-2.5">
                      <div className="flex gap-1.5 pt-2.5">
                        <button type="button" onClick={() => setAdjustType('restock')}
                          className={`flex-1 px-2.5 py-2 rounded-lg text-[12px] font-semibold border transition ${adjustType === 'restock' ? 'bg-emerald-50 text-emerald-700 border-emerald-300' : 'bg-[var(--color-surface-lowest)] text-[var(--color-botanical-muted)] border-[var(--color-botanical-border)]'}`}>
                          + Add Stock
                        </button>
                        <button type="button" onClick={() => setAdjustType('remove')}
                          className={`flex-1 px-2.5 py-2 rounded-lg text-[12px] font-semibold border transition ${adjustType === 'remove' ? 'bg-[#ffdad3] text-[#783020] border-[#edd1cc]' : 'bg-[var(--color-surface-lowest)] text-[var(--color-botanical-muted)] border-[var(--color-botanical-border)]'}`}>
                          − Remove
                        </button>
                      </div>
                      <div className="flex gap-2">
                        <input type="number" min="1" value={adjustQty} onChange={e => setAdjustQty(e.target.value)} placeholder="Qty"
                          className="flex-1 text-[13px] bg-[var(--color-surface-lowest)] border border-[var(--color-botanical-border)] focus:border-[#180f0a] rounded-lg px-3 py-2 text-[var(--color-botanical-text)] focus:ring-1 focus:ring-[#180f0a] transition" />
                        <input type="text" value={adjustReason} onChange={e => setAdjustReason(e.target.value)} placeholder="Reason"
                          className="flex-1 text-[12px] bg-[var(--color-surface-lowest)] border border-[var(--color-botanical-border)] focus:border-[#180f0a] rounded-lg px-3 py-2 text-[var(--color-botanical-text)] placeholder:text-[var(--color-botanical-subtle)] focus:ring-1 focus:ring-[#180f0a] transition" />
                      </div>
                      <button type="button" onClick={() => handleQuickAdjust(item)} disabled={!adjustQty || Number(adjustQty) <= 0 || savingId === item.productId}
              ...
              <div className="px-5 py-2.5 bg-[var(--color-surface-low)]">
                <div className="relative max-w-sm">
                  ...
                  <input type="search" value={historySearch} onChange={e => setHistorySearch(e.target.value)} placeholder="Search history..."
                    className="w-full text-[12px] bg-[var(--color-surface-lowest)] border border-[var(--color-botanical-border)] focus:border-[#180f0a] rounded-lg pl-8 pr-3 py-1.5 text-[var(--color-botanical-text)] placeholder:text-[var(--color-botanical-subtle)] focus:ring-1 focus:ring-[#180f0a] transition" />
                </div>
              </div>
